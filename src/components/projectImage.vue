	...
	<figure class="relative" :if="item.image.length ">
		<picture v-if="item.position === 'left top'">
			...
			<img
				...
				class="block object-left-top w-full h-full rounded-lg lg:rounded-l-lg object-fit dark:brightness-50 dark:hover:brightness-100 ring-ocean dark:ring-mirage ring-4"
		...
		<figcaption class="absolute px-2 py-1 text-xs text-white rounded-tr-lg rounded-br-lg rounded-bl-lg -left-1 -bottom-1 md:px-4 md:py-2 bg-ocean dark:bg-mirage dark:text-santa-grey " :if="item.image.length">
			{{ item.title }}
		</figcaption>
	</figure>
</template>
